...
"""
Marching Squares algorithm to compute the iso-valued contours of the input 2D array for a particular level value.
Note: still needs some reworking and optimisation. It would be good to add Douglas-Peuker line generalisation to
simplify the contours.

Originally written by @author: sushant
Modified by: Gabija Pasiunaite
"""
...
class MarchingSquareHandler:
    ...
    def compSval(self):
        ...
        for y in range(lenY):
            for x in range(lenX):
                self._scVal[x][y] = self.values[x][y]
    ...
    def getSquareData(self,n): #returns sqaures sv and vertices
        r=int(n//((self.winHeight//self.gridSize)-2))
        c=int(n%((self.winHeight//self.gridSize)-2))
    ...
    def compute(self):
        ...
        self.compSval()
        for i in tqdm(np.arange(0,_nSquare), desc='Computing vertices'):
            ...
            if self.checkifIntersects(_sv):
                ...
                if isSig[0]:
                    ...
                    _i1,_i2 =self.getIntersects(_sv) # ValueError: too many values to unpack
                    p1=self.intersectionPoint(_vert[_i1[0]],_vert[_i1[1]],self.isovalue,_sv[_i1[0]],_sv[_i1[1]])
                    p2=self.intersectionPoint(_vert[_i2[0]],_vert[_i2[1]],self.isovalue,_sv[_i2[0]],_sv[_i2[1]])
                self.linelist.append([p1,p2])
        print(np.array(self.linelist).shape)
        return np.array(self.linelist)
